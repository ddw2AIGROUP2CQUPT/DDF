...
% % p=getPrmDflt_x(varargin,dfs,1); % previous version 
% by xave
dfs={ 'modDir','resDir','REQ', 'gtDir','REQ', 'pDistr',{{'type','parfor'}}, ...
  'cleanup',0, 'thrs',99, 'maxDist',varargin{1,14}, 'thin',1,'use_nms'};
...
p.cleanup =0;
p.thrs = 99;
p.maxDist = varargin{1,14};
p.thin = 1;
% end by xavy
resDir=p.resDir; gtDir=p.gtDir;modDir = p.modDir;
evalDir=[modDir '/values']; if(~exist(evalDir,'dir')), mkdir(evalDir); end
...
gt_in_mat = length(dir(fullfile(gtDir,'*.mat')))>0;
gt_in_png = length(dir(fullfile(gtDir,'*.png')))>0;
if gt_in_mat
    ids=dir(fullfile(gtDir,'*.mat')); ids={ids.name}; n=length(ids); % gt names
...
elseif gt_in_png
    ...
    disp('it is not codded yet >>>> line 77 edgesEvalDir_x.m')
...
score_x =struct('name',{},'oisT1',{},'oisR1',{}, 'oisP1',{}, 'oisF1',{});
for i=1:n
  ...
  %by xavy
  [f_dir,f_name,f_ext] = fileparts(res{i});
  f_name = f_name(1:end-4);
  score_x(i).name=f_name;score_x(i).oisT1=num2str(oisT1, '%.4f');
  score_x(i).oisR1=num2str(oisR1, '%.4f');score_x(i).oisP1=num2str(oisP1, '%.4f');
  score_x(i).oisF1=num2str(oisF1, '%.4f');
  %end by xavy
...
% by xavy
writetable(struct2table(score_x), strcat(evalDir,'/ios_res.txt'))
% end by xavy
...
% eval_bdry.txt = [osdT, odsR, odsP, odsF, oisR, oisP, oisF, AP]
